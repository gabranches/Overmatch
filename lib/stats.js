...
module.exports = function () {
    ...
    me.uniqueObjArr = function(objArr, unique_field) {
        var newArr = [];
        var added = [];

        for (var i in objArr) {
            if (added.indexOf(objArr[i][unique_field]) == -1) {
                added.push(objArr[i][unique_field]);
    ...
    me.getMapStats = function (map, days) {
        ...
        for (var hero=0; hero < Object.keys(mapsByHero).length; hero++) {
                ...
                if (!(results[hero])) {
                    ...
                    if (mapsByHero[hero][map] && mapsByHero[hero][map].num_votes) {
        ...
        result = {};
        result['mapList'] = me.createArray(results, 0);    
        result['map'] = maps[map];
